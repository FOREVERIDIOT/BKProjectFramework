...
@interface BKCameraManager()<AVCaptureVideoDataOutputSampleBufferDelegate,AVCaptureAudioDataOutputSampleBufferDelegate>
...
-(void)switchCaptureDeviceComplete:(void (^)(BOOL flag, AVCaptureDevicePosition position))complete
{
    ...
    //切换到的摄像头
    AVCaptureDevicePosition resultPosition;
    //获取需要切换的摄像头
    if (currentPosition == AVCaptureDevicePositionBack) {
        resultPosition = AVCaptureDevicePositionFront;
    } else {
        resultPosition = AVCaptureDevicePositionBack;
    }
    //获取新的设备
    AVCaptureDevice * toDevice = [self getCameraDeviceWithPosition:resultPosition];
    if (!toDevice) {
        if (complete) {
    ...
    NSError * error = nil;
    //创建新的input
    AVCaptureDeviceInput * toInput = [AVCaptureDeviceInput deviceInputWithDevice:toDevice error:&error];
    if (error) {
        if (complete) {
            complete(NO, currentPosition);
        }
    ...
    if (resultPosition == AVCaptureDevicePositionFront) {
